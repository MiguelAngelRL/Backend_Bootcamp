// ################# EJERCICIO 3  ##############################################
// Crea una función que reciba una colección de objetos y un año. Dicha función 
// deberá de mostrar los nombres de las tres personas con el ranking más alto 
// del año.
...
const winnerByYear = (arr, year) => {
  const filteredArr = arr.filter(elem => elem.year===year);
  const sortedFilteredArr = filteredArr.sort(compare);
  const slicedSortedFilteredArr = sortedFilteredArr.slice(0,numberOfPeople);
  return slicedSortedFilteredArr.map((data) => data.name);
};
